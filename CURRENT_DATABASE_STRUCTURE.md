...
- **enrollments: insert** [INSERT] roles=(public) PERMISSIVE WITH CHECK ((is_course_creator(course_id) OR is_course_instructor(course_id) OR (course_id IN ( SELECT get_superadmin_course_ids() AS get_superadmin_course_ids)) OR ((user_id = auth.uid()) AND (EXISTS ( SELECT 1
   FROM (invites i
     JOIN profiles p ON ((p.id = auth.uid())))
  WHERE ((i.course_id = enrollments.course_id) AND (i.email = p.email) AND (i.status = 'pending'::invite_status)))))))
...
- **profiles: course peers read** [SELECT] roles=(public) PERMISSIVE USING ((id IN ( SELECT get_peer_user_ids() AS get_peer_user_ids)))
