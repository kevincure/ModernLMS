...
- `public.get_lti_deployment_for_course("p_org_id" "uuid", "p_registration_id" "uuid", "p_course_id" "uuid")`
...
- `public.send_reply_message("p_conversation_id" "uuid", "p_content" "text")`
...
|lineitem_url|"text"|NO|| |
|label|"text"|NO|| |
|score_max|numeric(10,2)|NO|| |
|resource_id|"text"|YES|| |
|tag|"text"|YES|| |
|lti_context_id|"text"|YES|| |
|created_by_launch_id|"uuid"|YES|| |
|created_at|"timestamp with time zone"|NO|now()| |
|updated_at|"timestamp with time zone"|NO|now()| |

**Foreign keys (outgoing):**

- "assignment_id" → public.assignments("id")  (constraint `lti_ags_line_items_assignment_id_fkey`)
- "course_id" → public.courses("id")  (constraint `lti_ags_line_items_course_id_fkey`)
- "created_by_launch_id" → public.lti_launches("id")  (constraint `lti_ags_line_items_created_by_launch_id_fkey`)
- "org_id" → public.orgs("id")  (constraint `lti_ags_line_items_org_id_fkey`)
- "registration_id" → public.lti_registrations("id")  (constraint `lti_ags_line_items_registration_id_fkey`)

**Policies (RLS):**

- **lti ags line items superadmin all** [ALL] roles=(public) PERMISSIVE USING (is_org_superadmin(org_id)) WITH CHECK (is_org_superadmin(org_id))


### `public.lti_ags_scores`
...
|user_id|"uuid"|YES|| |
|score_given|numeric(10,2)|YES|| |
|score_max|numeric(10,2)|YES|| |
|activity_progress|"text"|YES|| |
|grading_progress|"text"|YES|| |
|timestamp_from_tool|"timestamp with time zone"|YES|| |
|raw_payload|"jsonb"|NO|'{}'::jsonb| |
|created_at|"timestamp with time zone"|NO|now()| |

**Foreign keys (outgoing):**

- "line_item_id" → public.lti_ags_line_items("id")  (constraint `lti_ags_scores_line_item_id_fkey`)
- "org_id" → public.orgs("id")  (constraint `lti_ags_scores_org_id_fkey`)
- "user_id" → public.profiles("id")  (constraint `lti_ags_scores_user_id_fkey`)

**Policies (RLS):**

- **lti ags scores superadmin read** [SELECT] roles=(public) PERMISSIVE USING (is_org_superadmin(org_id))


### `public.lti_deep_link_items`
...
|deployment_id|"text"|YES|| |
|launch_id|"uuid"|YES|| |
|content_item_type|"text"|YES|| |
|content_title|"text"|YES|| |
|content_url|"text"|YES|| |
|custom_claims|"jsonb"|NO|'{}'::jsonb| |
|created_lms_type|"text"|YES|| |
|created_lms_id|"text"|YES|| |
|created_at|"timestamp with time zone"|NO|now()| |

**Foreign keys (outgoing):**

- "course_id" → public.courses("id")  (constraint `lti_deep_link_items_course_id_fkey`)
- "launch_id" → public.lti_launches("id")  (constraint `lti_deep_link_items_launch_id_fkey`)
- "org_id" → public.orgs("id")  (constraint `lti_deep_link_items_org_id_fkey`)
- "registration_id" → public.lti_registrations("id")  (constraint `lti_deep_link_items_registration_id_fkey`)

**Policies (RLS):**

- **lti deep link items superadmin read** [SELECT] roles=(public) PERMISSIVE USING (is_org_superadmin(org_id))


### `public.lti_deployments`
...
|scope_type|"text"|NO|| |
|scope_ref|"text"|YES|| |
|enable_deep_linking|"boolean"|NO|true| |
|enable_ags|"boolean"|NO|true| |
|enable_nrps|"boolean"|NO|true| |
...
- "org_id" → public.orgs("id")  (constraint `lti_deployments_org_id_fkey`)
- "registration_id" → public.lti_registrations("id")  (constraint `lti_deployments_registration_id_fkey`)

**Policies (RLS):**

- **lti deployments superadmin all** [ALL] roles=(public) PERMISSIVE USING (is_org_superadmin(org_id)) WITH CHECK (is_org_superadmin(org_id))


### `public.lti_key_rotation_events`
...
|cutover_at|"timestamp with time zone"|YES|| |
|result|"text"|NO|'scheduled'::text| |
|details|"jsonb"|NO|'{}'::jsonb| |
|created_at|"timestamp with time zone"|NO|now()| |

**Foreign keys (outgoing):**

- "initiated_by" → public.profiles("id")  (constraint `lti_key_rotation_events_initiated_by_fkey`)
- "org_id" → public.orgs("id")  (constraint `lti_key_rotation_events_org_id_fkey`)

**Policies (RLS):**

- **lti rotation events superadmin read** [SELECT] roles=(public) PERMISSIVE USING (is_org_superadmin(org_id))
...
|lti_version|"text"|YES|| |
|resource_link_id|"text"|YES|| |
|correlation_id|"text"|YES|| |
|status|"text"|NO|| |
|error_code|"text"|YES|| |
|error_detail|"text"|YES|| |
|raw_claims|"jsonb"|YES|| |
|created_at|"timestamp with time zone"|NO|now()| |

**Foreign keys (outgoing):**

- "course_id" → public.courses("id")  (constraint `lti_launches_course_id_fkey`)
- "org_id" → public.orgs("id")  (constraint `lti_launches_org_id_fkey`)
- "registration_id" → public.lti_registrations("id")  (constraint `lti_launches_registration_id_fkey`)
- "user_id" → public.profiles("id")  (constraint `lti_launches_user_id_fkey`)

**Policies (RLS):**

- **lti launches superadmin read** [SELECT] roles=(public) PERMISSIVE USING (is_org_superadmin(org_id))


### `public.lti_nrps_requests`
...
- "course_id" → public.courses("id")  (constraint `lti_nrps_requests_course_id_fkey`)
- "org_id" → public.orgs("id")  (constraint `lti_nrps_requests_org_id_fkey`)
- "registration_id" → public.lti_registrations("id")  (constraint `lti_nrps_requests_registration_id_fkey`)

**Policies (RLS):**

- **lti nrps requests superadmin read** [SELECT] roles=(public) PERMISSIVE USING (is_org_superadmin(org_id))


### `public.lti_platform_keys`
...
|org_id|"uuid"|NO|| |
|kid|"text"|NO|| |
|alg|"text"|NO|'RS256'::text| |
|use|"text"|NO|'sig'::text| |
|public_jwk|"jsonb"|NO|| |
|private_key_secret_ref|"text"|NO|| |
|status|"text"|NO|| |
...
- "org_id" → public.orgs("id")  (constraint `lti_platform_keys_org_id_fkey`)

**Policies (RLS):**

- **lti platform keys superadmin all** [ALL] roles=(public) PERMISSIVE USING (is_org_superadmin(org_id)) WITH CHECK (is_org_superadmin(org_id))


### `public.lti_registrations`
...
|issuer|"text"|NO|| |
|client_id|"text"|NO|| |
|auth_login_url|"text"|NO|| |
|auth_token_url|"text"|YES|| |
|jwks_url|"text"|YES|| |
|target_link_uri|"text"|YES|| |
|deep_link_return_url|"text"|YES|| |
|status|"text"|NO|'active'::text| |
...
- "org_id" → public.orgs("id")  (constraint `lti_registrations_org_id_fkey`)

**Policies (RLS):**

- **lti registrations superadmin all** [ALL] roles=(public) PERMISSIVE USING (is_org_superadmin(org_id)) WITH CHECK (is_org_superadmin(org_id))


### `public.lti_state_nonce`
...
- "org_id" → public.orgs("id")  (constraint `lti_state_nonce_org_id_fkey`)
- "registration_id" → public.lti_registrations("id")  (constraint `lti_state_nonce_registration_id_fkey`)

**Policies (RLS):**

- **lti state nonce superadmin read** [SELECT] roles=(public) PERMISSIVE USING (is_org_superadmin(org_id))
