...
- **notifications: insert** [INSERT] roles=(public) PERMISSIVE WITH CHECK (((user_id = auth.uid()) OR ((course_id IS NOT NULL) AND is_enrolled_in_course(course_id) AND (EXISTS ( SELECT 1
   FROM enrollments e
  WHERE ((e.course_id = notifications.course_id) AND (e.user_id = notifications.user_id)))))))
